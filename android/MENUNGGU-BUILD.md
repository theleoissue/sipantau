...
| menyusul | **Tanpa layar hitam saat APK dibuka** — splash ditahan sampai halaman pertama tampil (`MainActivity`, `onPageCommitVisible`, batas 4 detik), latar jendela dan WebView disetel `#F4F6F9` (`styles.xml`, `colors.xml`, `backgroundColor` di `capacitor.config.ts`) | Splash hilang begitu WebView kosong digambar, dan jeda sampai halaman dari Vercel tiba tampil hitam satu sampai dua detik — lebih lama bila server sedang bangun dari tidur |
